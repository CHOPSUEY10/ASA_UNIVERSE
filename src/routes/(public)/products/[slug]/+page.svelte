...
<div class="bg-brand-bg min-h-screen pt-12 pb-20 border-t border-brand-border text-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Breadcrumb -->
        <nav class="text-xs font-bold uppercase tracking-widest text-brand-muted mb-8 flex items-center">
            <a href="/" class="hover:text-brand-accent transition-colors">Home</a>
            <span class="mx-2 text-zinc-700">/</span>
            <a href="/products" class="hover:text-brand-accent transition-colors">Katalog</a>
            <span class="mx-2 text-zinc-700">/</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            <div>
                ...
                <div class="space-y-8 bg-brand-card border border-brand-border shadow-sm p-8 rounded-sm mb-8">
                    <h3 class="text-sm font-black uppercase tracking-wider border-b border-brand-border pb-3 mb-6">Customize Your Gear</h3>
                    
                    <!-- Design Upload -->
                    <div class="space-y-3">
                        <label class="block text-xs font-black uppercase tracking-wider text-gray-300">Upload Design <span class="text-brand-primary">*</span></label>
                        
                        {#if designFileUrl}
                            <div class="flex items-center justify-between p-4 bg-brand-bg border border-brand-border rounded-sm">
                                <div class="flex items-center space-x-3 overflow-hidden">
                                    {#if designFile?.type.startsWith('image/')}
                                        <div class="h-12 w-12 rounded-sm overflow-hidden bg-brand-surface flex-shrink-0 border border-brand-border">
                                            <img src={designFileUrl} alt="Design preview" class="h-full w-full object-cover" />
                                        </div>
                                    {:else}
                                        <div class="h-12 w-12 rounded-sm bg-brand-surface flex items-center justify-center text-zinc-600 flex-shrink-0 border border-brand-border">
                                    ...
                                    <div class="truncate">
                                        ...
                                        <a href={designFileUrl} target="_blank" class="text-xs font-black text-brand-accent uppercase tracking-wider hover:underline">View File</a>
                                    </div>
                                </div>
                                <div class="flex space-x-2">
                                    ...
                                    <button type="button" onclick={removeDesign} class="p-2 text-brand-primary hover:text-red-700 transition-colors" title="Hapus File">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        {:else}
                            <button 
                                ...
                                class="w-full border-2 border-dashed border-brand-border hover:border-brand-accent rounded-sm p-8 flex flex-col items-center justify-center transition-colors bg-brand-bg disabled:opacity-50 disabled:cursor-not-allowed group"
                            >
                                {#if isUploading}
                                    <svg class="animate-spin h-8 w-8 text-brand-primary mb-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                ...
                                {:else}
                                    <div class="w-12 h-12 rounded-full bg-brand-surface border border-brand-border flex items-center justify-center mb-4 group-hover:border-brand-primary transition-colors">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-zinc-500 group-hover:text-brand-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                                        </svg>
                                    </div>
                                    <span class="text-sm font-black text-white mb-1">Click to upload design</span>
                                    <span class="text-xs text-brand-muted">JPG, PNG, or PDF (Max 10MB)</span>
                                {/if}
                            </button>
                        {/if}
                ...
                <!-- Action Buttons -->
                <div class="fixed bottom-0 left-0 right-0 p-4 bg-brand-bg/95 backdrop-blur-md border-t border-brand-border z-50 flex gap-3 md:relative md:border-t-0 md:bg-transparent md:p-0 md:flex-row shadow-[0_-4px_20px_rgba(0,0,0,0.5)] md:shadow-none">
                    <button 
                        type="button" 
                        onclick={handleAddToCart}
                        class="flex-1 bg-brand-surface hover:bg-brand-card text-white border border-brand-border px-4 py-4 md:px-6 md:py-5 rounded-sm font-black uppercase tracking-widest transition-all text-center flex justify-center items-center gap-2 text-xs md:text-sm hover:shadow-[0_0_15px_rgba(255,255,255,0.05)]"
                    >
                    ...
                    <button 
                        ...
                        class="flex-[2] md:flex-1 bg-brand-accent hover:bg-yellow-500 text-brand-bg px-4 py-4 md:px-6 md:py-5 rounded-sm font-black uppercase tracking-widest transition-all text-center flex justify-center items-center gap-2 text-xs md:text-sm shadow-lg shadow-brand-accent/25 hover:shadow-brand-accent/40"
                    >
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946C.16 5.348 5.497.01 12.048.01c3.179.001 6.167 1.24 8.413 3.488 2.247 2.248 3.48 5.239 3.477 8.42-.005 6.545-5.343 11.883-11.89 11.883-2.006-.001-3.98-.51-5.74-1.48L0 24zm6.59-3.859c1.62.962 3.209 1.47 4.864 1.471 5.4 0 9.794-4.393 9.798-9.797.002-2.618-1.015-5.079-2.867-6.93-1.852-1.853-4.316-2.873-6.938-2.874-5.405 0-9.8 4.394-9.803 9.8-.001 1.734.464 3.427 1.346 4.92l.232.396-1.0 3.65 3.73-.979.37.221-.002.002zm11.124-7.464c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 0 0-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347"/>
                        </svg>
                        Buy Via Whatsapp
                    </button>
                </div>
