...
<div class="mb-6">
    <h3 class="text-xs font-bold text-gray-700 mb-4 uppercase tracking-wider">Kerah Model</h3>
        ...
        {#each kerahs as kerah}
            ...
            >
                ...
                <div class="bg-white px-2 py-2 text-center border-t border-gray-100">
                    <p class="text-[10px] font-bold text-gray-900 truncate uppercase tracking-widest">{kerah.nama}</p>
                    {#if kerah.harga}
                        <p class="text-[10px] text-gray-500 mt-1">Rp {kerah.harga.toLocaleString('id-ID')}</p>
                    {/if}
                </div>
            </button>
        {/each}
    ...
    {#if loadingImages}
        <p class="text-xs text-gray-500 mt-3 uppercase tracking-widest">Loading models...</p>
    {/if}
</div>
