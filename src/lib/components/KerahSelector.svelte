...
<div class="mb-6">
    <h3 class="text-sm font-medium text-gray-300 mb-4 uppercase tracking-wide">Pilih Model Kerah</h3>
        ...
        {#each kerahs as kerah}
            ...
            >
                ...
                <div class="bg-zinc-950/80 px-2 py-1 text-center">
                    <p class="text-xs font-medium text-gray-300 truncate">{kerah.nama}</p>
                    {#if kerah.harga}
                        <p class="text-[10px] text-gray-500">Rp {kerah.harga.toLocaleString('id-ID')}</p>
                    {/if}
                </div>
            </button>
        {/each}
