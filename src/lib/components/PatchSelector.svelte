...
<div class="mb-6">
    <h3 class="text-xs font-black text-gray-400 mb-4 uppercase tracking-wider">OFFICIAL PATCH COLLECTION</h3>
        ...
        {#each patches as patch}
            ...
            >
                ...
                <div class="bg-brand-card px-2 py-2.5 text-center border-t border-brand-border">
                    <p class="text-[10px] font-black text-white truncate uppercase tracking-widest">{patch.nama}</p>
                    {#if patch.harga}
                        <p class="text-[10px] text-brand-accent mt-1 font-bold">Rp {patch.harga.toLocaleString('id-ID')}</p>
                    {:else}
