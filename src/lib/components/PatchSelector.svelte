...
<div class="mb-6">
    <h3 class="text-xs font-bold text-gray-700 mb-4 uppercase tracking-wider">Jenis Patch Logo</h3>
        ...
        {#each patches as patch}
            ...
            >
                ...
                {#if selectedId === patch.id}
                    <div class="absolute inset-0 border-2 border-[#111] pointer-events-none"></div>
                {/if}

                <!-- Label -->
                <div class="bg-white px-2 py-2 text-center border-t border-gray-100">
                    <p class="text-[10px] font-bold text-gray-900 truncate uppercase tracking-widest">{patch.nama}</p>
                    {#if patch.harga}
                        <p class="text-[10px] text-gray-500 mt-1">Rp {patch.harga.toLocaleString('id-ID')}</p>
                    {/if}
                </div>
            </button>
        {/each}
    ...
    {#if loadingImages}
        <p class="text-xs text-gray-500 mt-3 uppercase tracking-widest">Loading patches...</p>
    {/if}
</div>
