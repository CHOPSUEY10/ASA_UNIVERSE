    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
            ...
            <div class="flex-shrink-0 flex items-center">
                <a href="/" class="text-2xl font-black tracking-tighter text-white flex items-center gap-2 transition-colors hover:text-brand-accent">
            ...
            <div class="flex items-center space-x-4">
                ...
                {#if !$session.data?.user}
                    <a href="/login" class="p-2 transition-colors {$page.url.pathname === '/login' ? 'text-brand-primary' : 'text-gray-300 hover:text-brand-accent'}" title="Login">
                ...
                {#if $session.data?.user}
                    {#if userRole === 'ADMIN' || userRole === 'admin'}
                        <a href="/admin" class="p-2 flex items-center transition-colors text-gray-300 hover:text-brand-accent" title="Admin Panel">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                            </svg>
                        </a>
                    {:else}
                        <a href="/profile" class="p-2 flex items-center transition-colors {$page.url.pathname === '/profile' ? 'text-brand-primary' : 'text-gray-300 hover:text-brand-accent'}" title="Pengaturan Akun">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                            </svg>
                        </a>
                    {/if}
                    <button onclick={handleLogout} title="Logout" class="p-2 flex items-center text-gray-300 hover:text-brand-accent transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                ...
                <a href="/cart" class="relative p-2 transition-colors {$page.url.pathname === '/cart' ? 'text-brand-accent' : 'text-gray-300 hover:text-brand-accent'}">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                    </svg>
                    {#if cartItemCount > 0}
                        <span class="absolute top-0 right-0 inline-flex items-center justify-center w-4 h-4 text-[10px] font-bold leading-none text-brand-bg transform translate-x-1/4 -translate-y-1/4 bg-brand-accent rounded-full">{cartItemCount}</span>
                    {/if}
                </a>
 
                <!-- Mobile menu button -->
                <button type="button" class="md:hidden p-2 transition-colors text-white hover:text-brand-accent focus:outline-none" onclick={toggleMenu} aria-label="Toggle menu">
